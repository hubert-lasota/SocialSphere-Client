...
interface UserService {
  ...
  findUserFriends: (userId: string, pageNumber: string, pageSize: string) => Promise<FriendPage>;
  findUser: (userId: string) => Promise<UserResponse>;
  updateUserProfile: (userProfile: UserProfile) => Promise<UserProfileResponse>;
  updateUserProfileConfig: (useProfileConfig: UserProfileConfig) => Promise<UserProfileConfigResponse>;
...
async function findUserFriends(userId: string, pageNumber: string, pageSize: string): Promise<FriendPage> {
  const finalUrl = url + "/friend";
...
async function updateUserProfile(userProfile: UserProfile): Promise<UserProfileResponse> {
  ...
  return (await fetchService.put(finalUrl, formData, urlParams, [tokenHeader])) as Promise<UserProfileResponse>;
}

async function updateUserProfileConfig(userProfileConfig: UserProfileConfig): Promise<UserProfileConfigResponse> {
  const userId = getFromLocalStorage("user_id");
  const tokenHeader = getJwtHeaderFromLocalStorage();
  const urlParams: UrlParameter[] = [{ key: "userId", value: userId }];
  const finalUrl = url + "/profile/config";

  return await fetchService.put(finalUrl, userProfileConfig, urlParams, [applicationJsonHeader, tokenHeader]);
}
