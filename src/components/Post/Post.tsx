...
import { faHeart as faHeartEmpty, faComments } from "@fortawesome/free-regular-svg-icons";
  ...
  return (
    <div key={id} className={`${styles["post"]} ${styles["post--bg-white"]}`}>
      <div className={styles["post__header"]}>
        <img src={profilePictureSrc} alt="profile" className={styles["header__profile-picture"]} onClick={() => handleGoOnUserPage(userId)} />
        <div className={styles["header__name"]}>
          <span className={`${styles["header__name__text"]} ${styles["header__name__text--fsmd"]} ${styles["header__name__text--grey"]}`}>{firstName}</span>
          <span className={`${styles["header__name__text"]} ${styles["header__name__text--fsmd"]} ${styles["header__name__text--grey"]}`}>{lastName}</span>
        </div>
      </div>
      ...
      <div className={styles["post__footer"]}>
        <div className={styles["footer__like"]}>
          <div className={styles["like__icon"]}>
            {isPostLiked ? (
              <FontAwesomeIcon icon={faHeartFilled} size="xl" onClick={() => handleRemoveLikeFromPost(id)} className={`${styles["like__filled-icon"]} ${styles["like__filled-icon--red"]}`} />
            ) : (
              <FontAwesomeIcon icon={faHeartEmpty} size="xl" onClick={() => handleAddLikeToPost(id)} className={styles["like__empty-icon"]} />
            )}
          </div>
          <span className={styles["like__text"]}>
            {likeCounter} {likeCounter === 1 ? "person likes" : "people like"} that post
          </span>
        </div>
        <div className={styles["footer__comment"]} onClick={() => handleOpenPostComments()}>
          <div className={styles["footer__comment__box"]}>
            ...
            <span className={styles["footer__comment__box__text"]}>{commentCount} comments</span>
          </div>
        </div>
      </div>
      {arePostCommentsOpen ? <PostCommentList postId={id}/> : <></>}
    </div>
  );
}
